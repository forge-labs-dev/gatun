...
public class ArrowMemoryHandler {
   ...
   *
   * <p>This manager implements proper reference counting so Arrow's lifecycle
   * management works normally. The actual memory is never freed since Python
   * owns it, but reference counting allows Arrow objects to clean up properly.
   *
   * <p>A single manager is created per batch/payload segment and shared by all
   * buffers in that batch to reduce object allocation overhead.
   */
   ...
   *
   * <p><b>Epoch validation:</b> The descriptor's arena_epoch must match our
   * current arenaEpoch. A mismatch indicates the sender is using stale buffer
   * offsets from before a reset, which would cause data corruption.
   *
   ...
   * <ul>
   *   <li>First half [0, size/2): Used by Python -> Java transfers</li>
   *   <li>Second half [size/2, size): Used by Java -> Python transfers</li>
   * </ul>
   * This avoids overwriting source data during bidirectional transfers.
   * See ArrowBatchDescriptor in commands.fbs for protocol documentation.
   *
   ...
   *
   * <p>Arrow IPC format uses depth-first pre-order traversal, so nested types
   * contribute additional nodes for their children. Each array (including
   * nested children) gets exactly one field node.
   *
   ...
   *
   * <p>Uses TypeLayout to get the exact buffer count for each type,
   * then recursively counts buffers for nested types.
   *
   * @param schema Arrow schema to analyze
